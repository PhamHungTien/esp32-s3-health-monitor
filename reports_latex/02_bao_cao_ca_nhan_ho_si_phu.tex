...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.72\textwidth,height=5.0cm,keepaspectratio]{assets/grayscale/max30102_board_overview.jpg}
...
(*@\VNCodeComment{Tính độ dốc của tín hiệu đã lọc so với mẫu ngay trước đó.}@*)
float slope = state.filtered - state.previousFiltered;
(*@\VNCodeComment{Đặt ngưỡng đỉnh bằng 95\% biên độ bao hiện tại để ngưỡng tự thích nghi với cường độ tín hiệu.}@*)
float peakThreshold = state.envelope * 0.95f;
(*@\VNCodeComment{Giới hạn ngưỡng tối thiểu ở 100 để nhiễu nhỏ không bị nhận nhầm thành nhịp.}@*)
if (peakThreshold < 100.0f) peakThreshold = 100.0f;
(*@\VNCodeComment{Dòng trống phân tách bước chuẩn bị ngưỡng với bước xác nhận đỉnh.}@*)

(*@\VNCodeComment{Phát hiện thời điểm độ dốc đổi từ âm sang không âm, tức một cực tiểu của dạng sóng đã đảo.}@*)
if (state.previousSlope < 0.0f && slope >= 0.0f &&
(*@\VNCodeComment{Yêu cầu biên độ cực tiểu vượt ngưỡng thích nghi vừa tính.}@*)
    state.previousFiltered < -peakThreshold &&
(*@\VNCodeComment{Buộc hai nhịp cách nhau hơn 450 ms để loại bỏ đỉnh giả quá gần.}@*)
    now - ppg.lastBeatMs > 450) {
(*@\VNCodeComment{Chỉ tính khoảng RR khi hệ thống đã lưu được thời điểm của một nhịp trước.}@*)
  if (ppg.lastBeatMs != 0) {
(*@\VNCodeComment{Tính khoảng thời gian giữa hai nhịp liên tiếp theo mili giây.}@*)
    uint32_t interval = now - ppg.lastBeatMs;
(*@\VNCodeComment{Chỉ chấp nhận khoảng RR từ 480 đến 1.500 ms, tương ứng vùng nhịp hợp lý của nguyên mẫu.}@*)
    if (interval >= 480 && interval <= 1500) {
(*@\VNCodeComment{Đổi khoảng RR sang nhịp mỗi phút bằng công thức 60.000 chia cho số mili giây.}@*)
      float instantBPM = 60000.0f / interval;
(*@\VNCodeComment{Kiểm tra đây có phải giá trị BPM hợp lệ đầu tiên của phiên đo hay không.}@*)
      if (!ppg.heartRateValid) {
(*@\VNCodeComment{Hiển thị trực tiếp BPM tức thời ở nhịp hợp lệ đầu tiên.}@*)
        ppg.bpm = instantBPM;
(*@\VNCodeComment{Đánh dấu BPM đã sẵn sàng để giao diện không tiếp tục hiện dấu chờ.}@*)
        ppg.heartRateValid = true;
(*@\VNCodeComment{Với các nhịp sau, chỉ nhận giá trị mới nếu chênh lệch không quá 22 BPM.}@*)
      } else if (fabsf(instantBPM - ppg.bpm) <= 22.0f) {
(*@\VNCodeComment{Làm mượt bằng 80\% giá trị cũ và 20\% giá trị mới để giảm rung số.}@*)
        ppg.bpm = 0.80f * ppg.bpm + 0.20f * instantBPM;
(*@\VNCodeComment{Kết thúc nhánh cập nhật BPM sau khi đã có kết quả trước đó.}@*)
      }
(*@\VNCodeComment{Kết thúc khối kiểm tra khoảng RR hợp lệ.}@*)
    }
(*@\VNCodeComment{Kết thúc khối yêu cầu phải có nhịp trước.}@*)
  }
(*@\VNCodeComment{Lưu thời điểm đỉnh hiện tại; đỉnh này sẽ làm mốc cho lần tính RR tiếp theo.}@*)
  ppg.lastBeatMs = now;
(*@\VNCodeComment{Kết thúc điều kiện xác nhận đỉnh nhịp.}@*)
}
\end{lstlisting}
