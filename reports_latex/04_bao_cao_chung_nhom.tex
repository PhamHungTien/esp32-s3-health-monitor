...
Ba thiết bị I2C được nhận đồng thời; màn hình hiển thị giao diện sức khỏe; MAX30102 phát hiện ngón tay và BPM hội tụ; IMU trả gia tốc nghỉ xấp xỉ 1$g$; GPS nhận NMEA, bắt FIX với 4 vệ tinh ở lần chạy cuối và xuất tọa độ. Kênh LEDC cho buzzer được khởi tạo trên GPIO42 và mã cảnh báo đã được cài đặt; âm lượng thực tế còn phụ thuộc loại loa và tầng công suất. Firmware biên dịch và nạp thành công lên ESP32-S3 N16R8. Đếm bước và té ngã đã có thuật toán nhưng vẫn cần thêm phép thử thực tế để xác định sai số.
\end{summarybox}
...
\begin{tikzpicture}[
  ...
  block/.style={draw=Primary,rounded corners,fill=SoftBlue,minimum width=31mm,minimum height=11mm,align=center,font=\small},
  core/.style={draw=Secondary,very thick,rounded corners,fill=Secondary!15,minimum width=40mm,minimum height=16mm,align=center,font=\bfseries},
...
Sketch không dùng Adafruit SSD1306/GFX, SparkFun MAX3010x, TinyGPS++, thư viện MPU hay thư viện xử lý tín hiệu. Vì vậy, việc đổi dữ liệu thô thành BPM, SpO$_2$, bước chân, trạng thái té ngã và tọa độ đều nằm trong mã nguồn của nhóm.
...
\CodeExplainLine{1}{Đọc một mẫu từ FIFO MAX30102 và lưu loại kết quả trả về.}
\CodeExplainLine{2}{Thoát vòng đọc khi FIFO hiện không có mẫu mới; đây không phải lỗi cảm biến.}
\CodeExplainLine{3}{Đi vào nhánh xử lý riêng khi giao dịch I2C với MAX30102 thất bại.}
\CodeExplainLine{4}{Đánh dấu cảm biến quang đang mất kết nối để cơ chế phục hồi có thể khởi tạo lại.}
\CodeExplainLine{5}{Xóa BPM, SpO$_2$ và trạng thái bộ lọc để không giữ số đo của phiên cũ.}
\CodeExplainLine{6}{Đặt cả hai mẫu quang thô về 0, tránh giao diện hiểu dữ liệu cũ là mẫu mới.}
\CodeExplainLine{7}{Dừng vòng đọc FIFO ngay sau lỗi bus.}
\CodeExplainLine{8}{Kết thúc nhánh xử lý lỗi giao tiếp.}
\end{codeexplanation}
...
\begin{codeexplanation}
\CodeExplainLine{1}{Kiểm tra hệ thống đã có một kết quả BPM hợp lệ trong phiên đo hiện tại hay chưa.}
\CodeExplainLine{2}{Ở nhịp hợp lệ đầu tiên, dùng trực tiếp BPM tức thời để giao diện hiện kết quả sớm.}
\CodeExplainLine{3}{Đánh dấu kết quả đã hợp lệ để các nhịp sau chuyển sang chế độ làm mượt.}
\CodeExplainLine{4}{Chỉ nhận BPM tiếp theo nếu nó lệch không quá 22 BPM so với giá trị đang hiển thị.}
\CodeExplainLine{5}{Kết hợp 80\% giá trị cũ với 20\% giá trị mới để giảm dao động giữa các nhịp.}
\CodeExplainLine{6}{Kết thúc nhánh lựa chọn giữa khởi tạo nhanh và cập nhật đã lọc.}
\end{codeexplanation}
...
\begin{codeexplanation}
\CodeExplainLine{1}{Chỉ xét một bước khi đã thấy đáy âm và tín hiệu động vượt ngưỡng dương 0,11 g.}
\CodeExplainLine{2}{Yêu cầu cách đỉnh trước hơn 280 ms và tổng gia tốc nhỏ hơn 1,8 g để loại rung hoặc va đập.}
\CodeExplainLine{3}{Tính thời gian giữa hai đỉnh; khi chưa có đỉnh trước thì trả về 0.}
\CodeExplainLine{4}{Xác nhận chu kỳ nằm trong miền 280--1.800 ms của một nhịp bước hợp lý.}
\CodeExplainLine{5}{Kiểm tra chuỗi đi bộ đã được xác nhận hay mới bắt đầu.}
\CodeExplainLine{6}{Nếu chuỗi đang hợp lệ, cộng thêm đúng một bước cho đỉnh mới.}
\CodeExplainLine{7}{Đi vào nhánh khởi tạo khi vừa có cặp đỉnh hợp lệ đầu tiên.}
\CodeExplainLine{8}{Cộng hai bước để tính cả đỉnh đầu tiên từng được giữ lại để xác nhận chuỗi.}
\CodeExplainLine{9}{Đánh dấu các đỉnh tiếp theo thuộc cùng một chuỗi đi bộ.}
\CodeExplainLine{10}{Kết thúc lựa chọn cách cập nhật bộ đếm bước.}
\CodeExplainLine{11}{Kết thúc điều kiện chu kỳ bước hợp lệ.}
\CodeExplainLine{12}{Kết thúc điều kiện xác nhận đỉnh dương.}
\CodeExplainLine{13}{Dòng trống phân tách thuật toán đếm bước với điều kiện phát cảnh báo té ngã.}
\CodeExplainLine{14}{Sau va đập, yêu cầu bất động hơn 1.800 ms và mức chuyển động dưới 0,08 g.}
\CodeExplainLine{15}{Đồng thời yêu cầu tổng gia tốc nằm gần 1 g, từ 0,72 đến 1,28 g.}
\CodeExplainLine{16}{Chuyển sang trạng thái cảnh báo khi các điều kiện sau va đập đều được thỏa mãn.}
\CodeExplainLine{17}{Kết thúc điều kiện xác nhận cảnh báo té ngã.}
\end{codeexplanation}
...
\begin{codeexplanation}
\CodeExplainLine{1}{Khởi tạo byte checksum tự tính bằng 0.}
\CodeExplainLine{2}{Duyệt phần thân NMEA, bắt đầu sau ký tự đô-la và dừng trước dấu sao.}
\CodeExplainLine{3}{XOR lần lượt từng ký tự để tạo checksum theo chuẩn NMEA 0183.}
\CodeExplainLine{4}{Kết thúc vòng lặp sau khi đã xử lý hết phần thân câu.}
\CodeExplainLine{5}{Ghép hai chữ số hexadecimal đã chuyển đổi thành checksum nhận kèm câu.}
\CodeExplainLine{6}{Trả về đúng khi byte tự tính trùng byte nhận được; câu sai checksum bị loại.}
\end{codeexplanation}
...
Hồ Sĩ Phú & MAX30102, FIFO, xử lý PPG, BPM, SpO$_2$, chất lượng và hiệu chuẩn & 33,3\%\\
...
Biên dịch & 358.067 byte flash (11\%), RAM 25.732 byte (7\%) & \OK\\
...
Nhóm đề xuất bổ sung nút SOS/hủy và động cơ rung; hiệu chuẩn SpO$_2$ bằng tập dữ liệu đối chứng; ghi lịch sử đo; dùng BLE/Wi-Fi/LTE để gửi cảnh báo; tối ưu tiêu thụ điện; thiết kế PCB và vỏ đeo. Đối với té ngã, ngưỡng cố định có thể được thay bằng mô hình thích nghi sau khi có bộ dữ liệu thử nghiệm đủ lớn.
