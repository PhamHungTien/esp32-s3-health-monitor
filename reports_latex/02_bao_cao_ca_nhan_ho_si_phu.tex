...
(*@\VNCodeComment{Đặt mức nền kênh hồng ngoại theo cùng nguyên tắc.}@*)
  state.dcIr = irValue;
(*@\VNCodeComment{Dừng xử lý mẫu vì không được phép tính BPM hoặc SpO$_2$ khi chưa có ngón tay.}@*)
  return;
...
(*@\VNCodeComment{Chỉ ước lượng SpO$_2$ khi tỷ số nằm trong miền 0,2 đến 2,0 để loại mẫu bất thường.}@*)
if (ratio > 0.2f && ratio < 2.0f) {
(*@\VNCodeComment{Áp dụng đường hiệu chuẩn tuyến tính của nguyên mẫu để đổi tỷ số quang sang phần trăm SpO$_2$.}@*)
...
(*@\VNCodeComment{Giới hạn kết quả trong khoảng 70--100\% để tránh giá trị hiển thị phi thực tế.}@*)
                                  70.0f, 100.0f);
(*@\VNCodeComment{Kiểm tra hệ thống đã có kết quả SpO$_2$ hợp lệ từ cửa sổ trước hay chưa.}@*)
...
(*@\VNCodeComment{Kết thúc nhánh cập nhật SpO$_2$.}@*)
}
\end{lstlisting}
