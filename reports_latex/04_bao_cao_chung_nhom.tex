...
Ba thiết bị I2C được nhận đồng thời; OLED và dashboard hiển thị dữ liệu sức khỏe; MAX30102 phát hiện ngón tay và BPM hội tụ; IMU trả gia tốc nghỉ xấp xỉ 1$g$; GPS bắt FIX với 4 vệ tinh ở lần chạy cuối. ESP32-S3 phát mạng \Code{health-monitor}, phục vụ giao diện tại 192.168.4.1 và API JSON cập nhật mỗi 0,5 giây. Kênh LEDC cho buzzer và hai thao tác web đặt lại bước/hủy cảnh báo đã được cài đặt. Đếm bước, SpO$_2$ và té ngã vẫn cần thêm hiệu chuẩn hoặc phép thử thực tế.
\end{summarybox}
...
\begin{figure}[htbp]
  ...
  \caption{Dashboard web tinh gọn được render với dữ liệu minh họa để kiểm tra bố cục. Khi chạy trên ESP32-S3, trang lấy dữ liệu thực từ API JSON mỗi 0,5 giây.}
...
MAX30102 cung cấp hai kênh quang RED/IR ở 100 Hz. Sau 0,12 giây bám nền, bộ dò theo dõi song song hai cực tính và chỉ khóa khi có một khoảng RR hợp lệ; BPM được xuất ngay từ khoảng đó. SpO$_2$ dùng tỷ lệ giữa AC/DC trong cửa sổ 64 mẫu, được xét độc lập với BPM và có thể xuất từ khoảng 0,76 giây nếu tín hiệu quang đạt điều kiện. Cờ hợp lệ ngăn màn hình dùng số khi chưa đủ dữ liệu.
...
Thời gian PPG tổng hợp & BPM 75 BPM không quá 1,75 giây; SpO$_2$ được xét từ khoảng 0,76 giây và không chờ BPM & \OK\\
...
Biên dịch & 968.902 byte flash (30\%), RAM 49.964 byte (15\%) & \OK\\
