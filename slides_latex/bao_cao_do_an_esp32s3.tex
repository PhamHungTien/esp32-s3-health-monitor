  ...
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.52\textwidth}
      \begin{itemize}\setlength\itemsep{3mm}
        ...
        \item API JSON cập nhật mỗi 0,5 giây, không chặn vòng lặp cảm biến
...
\begin{frame}{PPG chuyển RED/IR thành BPM có kiểm soát chất lượng}
  ...
  \bigmetric{0,76 s}{SpO$_2$ sớm nhất}
  \vspace{5mm}\par
...
\begin{frame}{IMU phục vụ đếm bước và máy trạng thái té ngã}
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{column}{0.50\textwidth}
      ...
      \flowstate{FREEFALL}{$<0{,}45g$}\par$\downarrow$\par
      \flowstate{IMPACT}{Va đập mạnh}\par$\downarrow$\par
      \flowstate{ALERT}{Ít chuyển động}
      \vspace{1mm}\par\tiny\muted{Thiếu điều kiện: quay về NORMAL.}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{Dashboard hiển thị toàn bộ trạng thái trên một màn hình}
  ...
  \vspace{-1mm}\par\tiny\muted{Bản render dùng dữ liệu minh họa; khi chạy thật, giao diện lấy JSON từ ESP32-S3 mỗi 0,5 giây.}
\end{frame}
...
\begin{frame}{Luồng dữ liệu từ cảm biến đến dashboard}
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.48\textwidth}
      ...
      \begin{enumerate}\setlength\itemsep{1.5mm}
        ...
        \item JavaScript cập nhật thẻ số đo mỗi 0,5 giây
  ...
  \end{columns}
  \vspace{1mm}\par
  \fcolorbox{Ink}{Ink}{\parbox{0.92\textwidth}{\centering\color{white}\small\bfseries Một ESP32-S3 duy trì PPG 100 Hz và dashboard 2 Hz; HTTP, DNS, JSON cùng toàn bộ giao diện đều do nhóm tự viết và lưu trong flash.}}
\end{frame}
